...
{
    //默认的参数
    int width=640,height=480,size=200,type=0;
    ...
    */
    ...
    viewer->addPointCloud(cloud,"cloud_in",v2);
...
void range_image2( pcl::PointCloud<pcl::PointXYZ>::Ptr pointCloud)
{
    ...
    float *ranges = rangeImage->getRangesArray();
    unsigned char* rgb_image = pcl::visualization::FloatImageUtils::getVisualImage(ranges,rangeImage->width,rangeImage->height);
    pcl::io::saveRgbPNGFile("/mrtstorage/users/students/chli/real_data/test_data2/rangeImage.png",rgb_image,rangeImage->width,rangeImage->height);
    std::cerr<<"rangeImage.png Saved!"<<std::endl;
